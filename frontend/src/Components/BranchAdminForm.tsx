...
type BranchAdminFormData = Omit<BranchAdmin, 'adminId'> & { adminId?: number };
...
const BranchAdminForm = () => {
    ...
    const defaultFormState: BranchAdminFormData = {
        username: '',
        email: '',
        name: '',
        address: '',
    ...
    const [branchAdmin, setBranchAdmin] = useState<BranchAdminFormData>(defaultFormState);
    ...
    useEffect(() => {
        ...
        if (mode === 'create') {
            setBranchAdmin(defaultFormState);
            setEditMode(false);
            ...
            if (editData && sessionId) {
                try {
                    ...
                    setBranchAdmin({
                        ...
                        userTypeId: parsed.userTypeId || 3,
                        branchId: branchId,
    ...
    useEffect(() => {
        const fetchDropdowns = async () => {
            ...
            try {
                const branchRes = await fetch('http://localhost:5008/api/branchadmins/branches', {
    ...
    useEffect(() => {
        if (editMode && !isInitialMount.current) {
            const editData = {
                adminId: branchAdmin.adminId,
                username: branchAdmin.username,
                email: branchAdmin.email,
                name: branchAdmin.name,
                address: branchAdmin.address,
                dob: formatDateForApi(branchAdmin.dob),
                userTypeId: branchAdmin.userTypeId,
                branchId: branchAdmin.branchId,
                contactNo: branchAdmin.contactNo,
    ...
    }, [branchAdmin, editMode, editSessionId]);

    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        const { name, value } = e.target;
        setBranchAdmin(prev => ({
            ...prev,
            [name]: name === 'branchId' || name === 'userTypeId' ? parseInt(value) || 0 : value,
        }));
    };

    const validateForm = () => {
        const { username, email, name, address, dob, branchId, contactNo, status } = branchAdmin;
        return (
            username.trim() &&
            email.trim() &&
            name.trim() &&
    ...
    const handleSubmit = async () => {
        ...
        confirm(
            editMode ? 'Update Branch Admin' : 'Create Branch Admin',
            editMode
                ? `Are you sure you want to update ${branchAdmin.name}?`
                : `Are you sure you want to create ${branchAdmin.name}?`,
            async () => {
                setLoading(true);
                try {
                    if (!token) {
                    ...
                    const dataToSend = {
                        ...
                        name: branchAdmin.name,
                        address: branchAdmin.address,
                        DOB: branchAdmin.dob,  // Note: Backend expects DOB (uppercase)
                        UserTypeId: 4,          // Fixed for BranchAdmin
                        BranchId: branchAdmin.branchId,
                        ContactNo: branchAdmin.contactNo,
                        Status: branchAdmin.status,
                    };
                    const url = editMode
                        ? `http://localhost:5008/api/branchadmins/${branchAdmin.adminId}`
                        : 'http://localhost:5008/api/branchadmins';
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(errorData.message || 'Failed to submit branch admin');
                    }

                    toast.success(editMode ? 'Branch Admin updated successfully!' : 'Branch Admin created successfully!', {
                        position: 'bottom-right',
                        autoClose: 3000,
                        onClose: () => {
                            resetForm();
                            if (editMode) {
                                navigate('/branch-admin-list');
                            }
                        },
                    });
                } catch (error: any) {
                ...
                <div className="max-w-2xl w-full bg-gradient-to-tr from-blue-200 to-white rounded-2xl shadow-lg p-8">
                    <div className="text-center mb-10">
                        ...
                        <h2 className="text-3xl font-semibold text-gray-800">
                            {editMode ? 'Edit Branch Admin' : 'Add New Branch Admin'}
                        </h2>
                        <p className="text-sm text-gray-600 mt-2">Branch Admin Information</p>
                    </div>

                    {editMode && (
                        <div className="mb-6 p-4 bg-blue-50 border-l-4 border-blue-400 rounded-md">
                            <h4 className="text-sm font-medium text-blue-800 mb-2">
                                Editing Branch Admin
                            </h4>
                            <p className="text-sm text-blue-700">
                                You are editing: <strong>{branchAdmin.name}</strong> (Username: {branchAdmin.username})
                            </p>
                        </div>
                    )}
